...
def drawPlotKde(data, title, x_axis, y_axis, size):
    h = [0.1, 0.3, 0.5]
    fig = plt.figure()
    st = fig.suptitle(title + ' n = ' + str(size), fontsize="x-large")
    for i in range(3):
        ax = fig.add_subplot(131 + i)
        ax.set_title('h = ' + str(h[i]))
        #plt.title('n = ' + str(size) + ', h = ' + str(h[i]))
        density = stat.gaussian_kde(data, bw_method=h[i])
        plt.plot(x_axis, density(x_axis), label='Kde')
        plt.plot(x_axis, y_axis, label='Pdf')
        plt.grid()
        plt.legend()
